...
CameraDevice::CameraDevice(const std::string &path, FormatInfo formatInfo,
    ...
    : IoMethod_(io), BufferSize_(bufferSize), FormatInfo_(formatInfo) {
  OpenDevice(path);
  InitDevice();
}
  ...
  case IOMethod::IO_METHOD_USERPTR:
    if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
      throw std::runtime_error(
          "[InitDevice] Device doesn't support streaming i/o");
    }
    break;
  }
...
void CameraDevice::InitMmapMode() {
  ...
  if (xioctl(FileDesc_, VIDIOC_REQBUFS, &req) == -1) {
    if (errno == EINVAL) {
      throw std::runtime_error(
          "[InitMmapMode] Device doesn't support memory mapping");
    } else {
  ...
  for (__u32 i = 0; i < req.count; ++i) {
    ...
    Buffers_[i].start =
        std::shared_ptr<void>(mmap(0, buf.length, PROT_READ | PROT_WRITE,
...
void CameraDevice::InitUserPtrMode(unsigned int bufSize) {
  ...
  if (xioctl(FileDesc_, VIDIOC_REQBUFS, &req) == -1) {
    if (errno == EINVAL) {
      throw std::runtime_error(
          "[InitUserPtrMode] Device doesn't support user pointer i/o");
    } else {
